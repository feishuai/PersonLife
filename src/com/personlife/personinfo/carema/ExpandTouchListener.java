...
class ExpandTouchListener implements View.OnTouchListener {
	...
	/**
	 * This is used to determine whether the user swipes from down to top.
	 * scrollUp is calculated from gesture detector scroll event. This shouldn't
	 * be used for the touch events
	 */
	...
	public static ExpandTouchListener newListener(Context context,
			AbsListView listView, View container, int gravity,
			int displayHeight, int defaultContentHeight) {
		return new ExpandTouchListener(context, listView, container, gravity,
				displayHeight, defaultContentHeight);
	}

	private ExpandTouchListener(Context context, AbsListView absListView,
			View container, int gravity, int displayHeight,
	...
	private void onTouchUp(View view, MotionEvent event) {
		...
		// if the dragging direction is from top to down and dialog position
		// still can't exceeds threshold
		// move the dialog automatically to top
		if (!touchUp && params.height < displayHeight
				&& params.height > (displayHeight * 4) / 5) {
		...
		// if the dragging direction is down to top and dialog is dragged more
		// than 50 to up
		// move the dialog automatically to top
		...
		// if the dragging direction is from down to top and dialog position
		// still can't exceeds threshold
		// move the dialog automatically to down
		if (touchUp && params.height <= defaultContentHeight + 50) {
			Utils.animateContent(contentContainer, defaultContentHeight,
					new SimpleAnimationListener());
			return;
		}

		// if the dragging direction is from top to down and the position
