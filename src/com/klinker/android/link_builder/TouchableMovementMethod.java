...
public class TouchableMovementMethod extends LinkMovementMethod {
	 ...
	 * @param spannable
	 *            spannable string inside the clicked view
	 * @param event
	 *            motion event that occurred
	 * @return
	 */
	@Override
	public boolean onTouchEvent(final TextView textView,
			final Spannable spannable, MotionEvent event) {
	 ...
	 */
	private TouchableSpan getPressedSpan(TextView widget, Spannable spannable,
			MotionEvent event) {
		...
		if (off != end && off != end - 1) {
			TouchableSpan[] link = spannable.getSpans(off, off,
					TouchableSpan.class);
	...
	/**
	 * Don't need to create a new instance for every text view. We can re-use
	 * them
	 * 
