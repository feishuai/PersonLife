...
public class LinkBuilder {
	...
	public static LinkBuilder from(Context context, String text) {
		return new LinkBuilder(TYPE_TEXT).setContext(context).setText(text);
	}

	public static LinkBuilder on(TextView tv) {
		return new LinkBuilder(TYPE_TEXT_VIEW).setContext(tv.getContext())
				.setTextView(tv);
	}
	 ...
	 *
	 * @param link
	 *            the rule that you want to link with.
	 */
	 ...
	 *
	 * @param links
	 *            list of rules you want to link with.
	 */
	...
	public CharSequence build() {
		...
		if (type == TYPE_TEXT_VIEW) {
			...
			// add the movement method so we know what actions to perform on the
			// clicks
	 ...
	 *
	 * @param s
	 *            spannable text that we are adding the rule to.
	 * @param link
	 *            rule to add to the text.
	 */
	private void addLinkToSpan(Spannable s, Link link) {
		...
		while (matcher.find()) {
			...
			// find the start and end point of the linked text within the
			// TextView
	...
	private void addLinkMovementMethod() {
		...
		if ((m == null) || !(m instanceof TouchableMovementMethod)) {
			if (textView.getLinksClickable()) {
				textView.setMovementMethod(TouchableMovementMethod
						.getInstance());
			}
		}
	}
	 ...
	 *
	 * @param link
	 *            rule we are applying.
	 * @param range
	 *            the start and end point of the link within the text.
	 * @param text
	 *            the spannable text to add the link to.
	 */
	private void applyLink(Link link, Range range, Spannable text) {
		TouchableSpan span = new TouchableSpan(context, link);
		text.setSpan(span, range.start, range.end,
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	}

	/**
	 * Find the links that contain patterns and convert them to individual
	 * links.
	 */
	...
	/**
	 * Add the appended and prepended text to the links and apply it to the
	 * TextView so that we can create the SpannableString.
	 */
	private void applyAppendedAndPrependedText() {
		for (int i = 0; i < links.size(); i++) {
			...
			if (link.getPrependedText() != null) {
				String totalText = link.getPrependedText() + " "
						+ link.getText();
			...
			if (link.getAppendedText() != null) {
				String totalText = link.getText() + " "
						+ link.getAppendedText();
